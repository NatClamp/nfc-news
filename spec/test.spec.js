  ...
  describe('/api', () => {
      ...
      describe('/:topics/articles', () => {
        ...
        it("ERROR - responds with 422 if the client provides a user_id that doesn't exist", () => {
          const newArticle = {
            ...
            user_id: 799,
            body:
              'Every day coding is like experience an entire life in 24 hours',
          };
          return request
            .post('/api/topics/mitch/articles')
            .send(newArticle)
        ...
        describe('/comments', () => {
          ...
          it("ERROR - responds with 422 if client enters a user_id that doesn't exist", () => {
            const newComment = {
              user_id: 987,
              body:
                'Were Shibes doing all these funny things before people started taking photos and posting them online?',
            };
            return request
              .post('/api/articles/1/comments')
              .send(newComment)
    ...
    describe('/users', () => {
      ...
      describe('/:user_id', () => {
        ...
        it('ERROR - responds with 400 if the client enters a username in the incorrect syntax', () => request
          .get('/api/users/1')
          .expect(400)
          .then((res) => {
            expect(res.body.message).to.equal(
