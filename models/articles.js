...
exports.getArticles = (req, res, next) => {
  ...
  if (Number.isInteger(+req.params.topic)) {
    return next({ code: 42703, message: 'Invalid format' });
  }
  if (!Number.isInteger(+limit) || !Number.isInteger(+p)) {
    return next({ code: '22P02', message: 'invalid input syntax for integer' });
  }
  if (+p < 0) {
    return next({ code: '2201X', message: 'OFFSET must not be negative' });
  }
  const validSorts = [
    'article_id',
    'title',
